...
var APP = {
	...
	Player: function () {
		...
		this.load = function ( json ) {
			...
			var baseMap2 = THREE.ImageUtils.loadTexture('osnova_map.png');
			...
			var pendulumOp = THREE.ImageUtils.loadTexture('opacity_pendulum.png');
			var pendulumLight = THREE.ImageUtils.loadTexture('light_pendulum.png');
			var gear1_map = THREE.ImageUtils.loadTexture('gear_2_map.png');
			var gear3_map = THREE.ImageUtils.loadTexture('gear_1_map.png');
			var gear2_map = THREE.ImageUtils.loadTexture('gear_3_map.png');
			var gear2_bumpMap = THREE.ImageUtils.loadTexture('gear_3_bump.png');
			var smMap = THREE.ImageUtils.loadTexture('sm.jpg');
			var smOpMap = THREE.ImageUtils.loadTexture('sm_op_1.png');
			var smBumpMap = THREE.ImageUtils.loadTexture('sm_bump.png');
			var smMap2 = THREE.ImageUtils.loadTexture('sm_map_2.png');
			var smOpMap2 = THREE.ImageUtils.loadTexture('sm_op_2.png');
			var smlightMap = THREE.ImageUtils.loadTexture('sm_bump_1.png');
			
			var basement=scene.getObjectByName('basement_child', true)
			var basement_=scene.getObjectByName('basement', true)
			var pendulum=scene.getObjectByName('pendulum', true)
			var gear_1=scene.getObjectByName('gear_1', true)
			var gear_3=scene.getObjectByName('gear_3', true)
			var gear_2=scene.getObjectByName('gear_2', true)
			var type_plate=scene.getObjectByName('type_plate', true)
			var _type=scene.getObjectByName('type', true)
			
			_type.material.map = smMap2
			_type.material.alphaMap = smOpMap2
			type_plate.material.map = smMap
			type_plate.material.alphaMap = smOpMap
			type_plate.material.specularMap = smlightMap
			//type_plate.material.lightMap = smlightMap
			//type_plate.material.bumpMap = smBumpMap
			pendulum.material.map = pendulumMap
			pendulum.material.alphaMap = pendulumOp
			pendulum.material.specularMap = pendulumLight
			pendulum.material.bumpMap = pendulumBump
			gear_1.material.map = gear1_map
			gear_1.material.bumpMap = gear1_map
			gear_2.material.map = gear2_map
			gear_2.material.bumpMap = gear2_bumpMap
			gear_3.material.map = gear3_map
			gear_3.material.bumpMap = gear3_map
			//pendulum.material.bumpScale = 0.5
			basement_.material.map = baseMap2
			basement_.material.bumpMap = baseMap2
			basement_.material.bumpScale = 0.1
			...
			clock.scale.copy(new THREE.Vector3( 0.001, 0.001, 0.001 ));
		...
		var motion = function (){
			...
			robject.forEach(function(_object, index){
				if (clockFixed != true) {
					...
					//_object.rtween.stop();
					if (_object.name == 'type_plate' && target.rz<-0.1) {
						console.log(target.rx, target.ry, target.rz)
						target.rz=-0.1;
					}
					if (_object.name == 'type_plate' && target.rx<-0.2) {
						target.rx=-0.2;
					}
					if (_object.name == 'type_plate' && target.rx>0.3) {
						//console.log(target.rx, target.ry, target.rz)
						//console.log(true);
						target.rx=0.3;
					}
...
*/
		...
		var clockFlyToCamera = function(callback){
			...
			_clock.position.copy(new THREE.Vector3( 0, 350, 0 ));
			_clock.scale.copy(new THREE.Vector3( 1, 1, 1 ));
		...
		var canMoveType = function(callback){
			arr = [
				scene.getObjectByName('type_plate'),
			]
			canMoveFunc(arr, callback)
		}
		...
		var canMoveGears = function(callback){
			arr = [
				scene.getObjectByName('gear_1_box'),
				scene.getObjectByName('gear_2_box'),
				scene.getObjectByName('gear_3_box')
			]
			canMoveFunc(arr, callback)
		}
		...
		var lose = function(){
			$('#steps > *:not(.step-lose)').fadeOut();
			//$('.step-2').fadeOut();
			$('.step-lose').fadeIn();
			$('#steps .step-lose .btn').bind('click touchstart', function(e){
		...
		scenario = function(){
			$('#steps .step-1').fadeIn(300, function(){
				$('#steps .step-1 .btn').bind('click touch', function(e){
					e.preventDefault();
													...
													step3(function(){
														setTimeout(function(){
															$('.step-3').fadeOut(300, function(){
																...
																var _type = scene.getObjectByName('type_plate');
																animateOpacity(_type, {'opacity': 1}, 500);
																$('.step-5').fadeIn();
																canMoveType(function(){
																	clockFixed = true;
																	var _type = scene.getObjectByName('type');
																	_type.rotation.x=0
																	_type.rotation.y=0
																	_type.rotation.z=0
																	animateRTween(_type, undefined, undefined, undefined, function(){
																	moveHands();
																	})
																});
																//$('#pachka').addClass('show');
																//startClock(function(){});
															});
														}, 2000);
													})
		...
		function rotateToZero2(_object) {
			...
			animateRTween(_object, target, 50, undefined, undefined, false);
		}
		
		function fixOnPosition(_object) {
			var minClose = 20;
			if (in_array(_object, canMove.arr)) {
				if (_object.name=="type_plate") {
					minClose = 70;
				}
				var condition = Math.abs(_object.position.x)<=minClose && Math.abs(_object.position.y)<=minClose && Math.abs(_object.position.z)<=minClose
				...
				if (condition) {
					...
					untouch();
					if (_object.name=="type_plate") {
						animateOpacity(_object, {'opacity': 0}, 500, callback = function(){
							var _type = scene.getObjectByName('type', true);
							animateOpacity(_type, {'opacity': 1}, 500);
							animateRTween(_type);
						});
		...
		var crashAll = function(callback) {
			callback = typeof callback !== 'undefined' ? callback : function(){};
			objects.forEach(function(_object, index){
				if (_object.userData.position && _object.name!='type') {
		...
		var animatePosYTween = function(_object, target, time, easing, callback, animate_set){
			
			target = typeof target !== 'undefined' ? target : {y:0};
			time = typeof time !== 'undefined' ? time : 100;
			easing = typeof easing !== 'undefined' ? easing : TWEEN.Easing.Linear.None;
			callback = typeof callback !== 'undefined' ? callback : function(){};
			animate_set = typeof animate_set !== 'undefined' ? animate_set : true;
		...
		var onDocumentMouseDown = function (event) {
			...
			if ( intersects.length > 0) {
				...
				if (SELECTED.name != 'basement' && SELECTED.userData.fixed != true) {
					...
					/*if (SELECTED.rotation.x!=0 && SELECTED.rotation.y!=0 && SELECTED.rotation.z!=0) {
						console.log('!!!')
					}*/
					objects.forEach(function(_object, index){
						if (_object.userData.fixed == true) {
				...
				/*if (SELECTED.name == 'type_plate') {
					animatePosYTween(SELECTED, {
							y: 10,
					}, 100, TWEEN.Easing.Linear.None, function(){}, false)
				}*/
		...
		var onDocumentMouseMove = function (event) {
			...
			if ( SELECTED ) {
				...
				if (SELECTED.rotation.x!=0 || SELECTED.rotation.y!=0 || SELECTED.rotation.z!=0) {
					SELECTED.rotation.x=0
					SELECTED.rotation.y=0
					SELECTED.rotation.z=0
					//rotateToZero2(SELECTED);
					//animateRTween(SELECTED, undefined, 10, undefined, undefined, false);
				}
				if (SELECTED.position.y<5 && SELECTED.name != "type_plate") {
